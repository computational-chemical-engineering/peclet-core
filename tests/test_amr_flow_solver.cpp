...
void test_sphere_ghostproj_adv() {
  ...
  // mode: 0 = explicit aperture, 1 = explicit ghost, 2 = DEFAULT (nothing set — since the
  // 2026-08-25 default switch, AUTO must resolve to the GHOST projection on this well-banded
  // mesh).
  auto setup = [&](auto& f, int mode) {
    ...
    if (mode == 1)
      ...
      f.setGhostProjection(false);  // explicit aperture
    f.setSolid(sdf);
  };
  oracle::AmrFlow<21> hfl;
  setup(hfl, 1);
  ...
  setup(afl, 0);
  AmrFlow<21> ufl;  // DEFAULT: AUTO must resolve to the ghost projection (2026-08-25 switch)
  setup(ufl, 2);
  for (int s = 0; s < 50; ++s) {
    hfl.step(300, 12, 2);
    dfl.step(200, 60);
    afl.step(200, 80);
    ufl.step(200, 60);  // same budgets as dfl: the default must be bit-identical to it
  }
  const auto& hux = hfl.velocity(0);
  const auto dux = dfl.velocity(0);
