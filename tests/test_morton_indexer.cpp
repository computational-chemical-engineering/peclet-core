// Serial correctness of the Morton/Z-order cell indexer (tpx::decomp::MortonIndexer):
...
#include "tpx/decomp/morton_indexer.hpp"
...
using namespace tpx;
using tpx::decomp::MortonIndexer;
...
static void test3d() {
  ...
  forEachInBox<3>(bgn, end, [&](const IVec<3>& g) {
    ...
    TPX_CHECK_EQ((long long)c, (long long)ref.code());

    // Round-trips back to the same global multi-index.
    IVec<3> g2 = idx.multiIndex(c);
    TPX_CHECK(g2[0] == g[0] && g2[1] == g[1] && g2[2] == g[2]);

    // Bijection: no two cells share a code.
    TPX_CHECK(seen.insert(c).second);
  });
  TPX_CHECK_EQ((long long)seen.size(), (long long)(size[0] * size[1] * size[2]));
  ...
  for (int axis = 0; axis < 3; ++axis) {
    ...
    for (int k = 0; k < 3; ++k) {
      TPX_CHECK_EQ((long long)up[k], (long long)(g[k] + (k == axis ? 1 : 0)));
      TPX_CHECK_EQ((long long)dn[k], (long long)(g[k] - (k == axis ? 1 : 0)));
    }
  }
}
...
static void test2d() {
  ...
  forEachInBox<2>(IVec<2>{0, 0}, IVec<2>{8, 8}, [&](const IVec<2>& g) {
    ...
    TPX_CHECK(g2[0] == g[0] && g2[1] == g[1]);
    TPX_CHECK(seen.insert(c).second);
  });
  TPX_CHECK_EQ((long long)seen.size(), 64LL);
}
...
#else
int main() {
  std::printf("TPX_HAVE_MORTON not set — skipping morton indexer test\n");
  return 0;
}
