...
// `tpx::Field3D<T>` of logical shape (nx,ny,nz) exports with element strides {1, nx, nx*ny} — i.e.
// a Fortran-order NumPy array indexed [x,y,z], matching docs/CONVENTIONS.md §6.
#ifndef TPX_PYTHON_NDARRAY_INTEROP_HPP
#define TPX_PYTHON_NDARRAY_INTEROP_HPP
...
/// Import a (contiguous) Python array into a freshly-allocated flat device `tpx::View<T>`. A host
/// array is staged up (deep_copy from an unmanaged host wrap == the old `tpx::toDevice`); a device
...
template <class T>
tpx::View<T> ndarray_to_view(const nb::ndarray<>& a, const std::string& label) {
  require_dtype<T>(a, "ndarray_to_view");
  const std::size_t n = a.size();
  tpx::View<T> d(Kokkos::view_alloc(label, Kokkos::WithoutInitializing), n);
  ...
  } else if constexpr (!is_host_space_v<tpx::MemSpace>) {
    if (dev != dlpack_device<tpx::MemSpace>().first)
      throw std::runtime_error("ndarray_to_view: array is on an incompatible device for this build");
    Kokkos::View<const T*, tpx::MemSpace, Kokkos::MemoryTraits<Kokkos::Unmanaged>> dv(
        static_cast<const T*>(a.data()), n);
...
#endif  // TPX_PYTHON_NDARRAY_INTEROP_HPP
