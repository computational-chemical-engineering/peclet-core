...
//
// Lifted verbatim from the AMR tree (peclet/core/amr/face_csr.hpp, 2026-09-10; QUALITY_PLAN G.2) —
// the AMR package and voro's mesh optimiser both consume it. The "host" serial reference solver
// (peclet::amr::AmrCutCell / oracle::AmrFlow, pure C++20) and the "device" Kokkos solver
// (MomentumOp + csr_operator.hpp) used to carry two independent encodings of the SAME per-cell
// arithmetic — a real drift risk. This header is the single source of that arithmetic: the
// assembled operator
//
//     (A u)_i = diag_i · u_i + Σ_{k ∈ [start_i, start_{i+1})} coef_k · u[nbr_k]
//                              ( + optional implicit-FOU advection over a second CSR )
//
// expressed once as PECLET_HD row kernels over a *templated accessor*, so the SAME body serves
//   - the host: a plain serial loop, arrays = raw pointers (no Kokkos dependency — PECLET_HD is
//     plain `inline` in a non-Kokkos, non-CUDA build, so these compile as ordinary functions); and
...
// kernel layer for a Voronoi-cell (unstructured polyhedral) operator.
#ifndef PECLET_CORE_SOLVER_FACE_CSR_HPP
#define PECLET_CORE_SOLVER_FACE_CSR_HPP

#include "peclet/core/common/types.hpp"

// PECLET_HD (common/portable.hpp): KOKKOS_INLINE_FUNCTION under a Kokkos build, __host__ __device__
// inline under nvcc/hipcc, plain inline otherwise — so the row kernels are device-callable when
// compiled for the device and ordinary host functions in the pure-C++ build.
...
struct HostArr {
  ...
  PECLET_HD T operator()(Index i) const { return p[i]; }
};
...
PECLET_HD double faceCsrApplyRow(const Op& op, Index i, const U& u) {
...
template <class Op, class U>
PECLET_HD void faceCsrOffDiag(const Op& op, Index i, const U& u, double& off, double& d) {
  off = 0.0;
  d = op.diag(i);
  for (Index k = op.start(i); k < op.start(i + 1); ++k)
...
PECLET_HD double faceCsrPointUpdate(double b_i, double off, double d, double uOld, double omega) {
...
template <class Op, class U>
PECLET_HD double fvApplyRow(const Op& op, Index i, const U& u) {
  const double ui = u(i);
  double acc = 0.0;
  for (Index k = op.start(i); k < op.start(i + 1); ++k)
...
template <class Op, class U>
PECLET_HD double fvPointSolve(const Op& op, Index i, const U& u, double rhs_i, double uOld) {
...
#endif  // PECLET_CORE_SOLVER_FACE_CSR_HPP
