...
class ParticleHalo {
 public:
  ...
  void forward(const View<T>& owned, const View<T>& ghost, int tag = 7603) {
    ...
    View<T> sendBuf = scratchAs<T>("peclet::core::halo::p_sendBuf");
    if (nSend_) {
      View<T> o = owned;
      IndexView idx = d_sendIdx_;
      View<T> buf = sendBuf;
      Kokkos::parallel_for(
          "peclet::core::halo::p_gather", Kokkos::RangePolicy<ExecSpace>(0, nSend_),
    ...
    if (numSelf_) {
      ...
      Kokkos::parallel_for(
          "peclet::core::halo::p_selfGather", Kokkos::RangePolicy<ExecSpace>(0, numSelf_),
  ...
  void reverse(const View<T>& ghost, const View<T>& owned, int tag = 7604) {
    ...
    if (nSend_) {
      ...
      Kokkos::parallel_for(
          "peclet::core::halo::p_scatter", Kokkos::RangePolicy<ExecSpace>(0, nSend_),
    ...
    if (numSelf_) {
      ...
      const Index base = numReceived_;
      Kokkos::parallel_for(
          "peclet::core::halo::p_selfScatter", Kokkos::RangePolicy<ExecSpace>(0, numSelf_),
...
#endif  // PECLET_CORE_HALO_PARTICLE_HALO_HPP
