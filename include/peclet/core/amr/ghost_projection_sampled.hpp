...
inline GhostOverlaySampled buildGhostOverlaySampled(const BlockOctree<3, Bits>& t,
  ...
  // decomposition.
  //
  // The bins survive DD1 only as that order key, but `domain` below is also the minimum-image
  // PERIOD of the cloud metric, so it has to be the true one: F2 (§Findings, fixed on main before
