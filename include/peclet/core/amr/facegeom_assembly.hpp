...
//
// buildFaceGeom (flow.hpp) walks AmrPoisson::forEachFaceFull on the host to produce the
// per-(sub)face geometry CSR the collocated divergence / gradient / ABC face-field / FOU advection
// kernels consume: neighbour, axis, dir, α·area, raw area, distance, openness α, and the two SOU
// upstream-probe leaves. It is pure per-face geometry — embarrassingly parallel — so for a moving
// boundary it should rebuild on device rather than re-walk the host and re-upload. This header does
// that, reusing the D2 FvFaceEmit traversal helpers (decode / wrap / areaOf / openness / locate)
// and the S1 offsets primitive.
//
// Bit-exactness: same forEachFaceFull enumeration (axis-major, dir −1/+1, 2:1 sub-faces), same
// per-face formulas, each cell fills its own slice — so on OpenMP the device FaceGeom == host
...
/// h0 + fineExt (built with hasOpen as the source openness has it).
...
FaceGeom assembleFaceGeom(const AmrPoisson<3, Bits>& ap, const std::vector<char>& fluidHost,
  ...
  for (int d = 0; d < 3; ++d)
    emit.g.fineExt[d] = ap.fineExt()[d];
  if (ap.hasOpenness())
    emit.g.alpha = toDevice(ap.opennessRaw(), "fg::alpha");

  const Index n = ov.numLeaves();
  Index nf = 0;
  View<Index> start = scanOffsets(n, KOKKOS_LAMBDA(const Index i) { return emit.count(i); }, nf);
  ...
  Kokkos::parallel_for(
      "amr::facegeom_fill", n, KOKKOS_LAMBDA(const Index i) {
        ...
        struct FillCB {
          ...
          Index i;
          Index k;
          KOKKOS_INLINE_FUNCTION void operator()(Index j, int axis, int dir, double area,
