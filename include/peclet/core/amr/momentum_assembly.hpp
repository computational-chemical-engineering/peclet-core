...
//
// AmrCutCell::build (host) samples the SDF per cell and runs buildCutStencil to produce the
// ξ-overlay stencil (AC_/off_/cut_/rscale_); assembleOperator (host) then merges that stencil, the
// regular-fluid C/F-aware ∇², and the implicit-FOU advection into one diag + face-CSR via a
// std::vector<vector<pair>>. For a moving boundary that whole walk reruns every step on the host
// and round-trips the operator.
//
// This header moves the two device-portable halves onto the device:
//   * the per-cell ξ stencil rebuild — buildCutStencil is pure math (now MORTON_HD), fed the cell +
//   6
//     neighbour SDF samples (staged like D2 stages openness; the SDF *sampling* itself, like
//     openness sampling, stays host until a device SDF exists); and
//   * the operator merge — the three per-cell branches (solid identity / ξ-overlay / regular ∇²·μ)
//   plus
//     the advection fold, emitted as a face-CSR through buildFaceCsr (S1) + a per-cell diagonal.
// The regular-fluid faces reuse the D2 FvFaceEmit traversal (α=1) via a scaling sink adapter, so
// the 2:1 enumeration and coeff are shared, not re-derived.
//
// Bit-exactness: emit order, branch logic and arithmetic mirror assembleOperator exactly, each cell
// fills its own CSR slice (S1, atomic-free), so on OpenMP the device MomentumOp == host
...
/// Device-callable reproduction of AmrCutCell::assembleOperator's per-cell row + diagonal. Holds
/// the staged stencil/advection device arrays + the shared α=1 ∇² geometry (FvFaceEmit). Trivially
/// copyable.
...
struct MomFaceEmit {
  ...
  KOKKOS_INLINE_FUNCTION void operator()(Index i, Sink& sink) const {
    if (!fluid(i))
      return;  // solid: identity row, no off-diagonals
    ...
    if (hasAdv) {
      ...
      for (Index p = advStart(i); p < advStart(i + 1); ++p)
        sink(advNbr(p), as * advCoef(p));
    }
  }

  /// Diagonal of row i: 1 (solid), AC (cut), or idiag + μ·invV·Σ(a·c) (regular), plus folded
  /// advection.
...
/// indices + fluid flag, recompute AC/off/cut/rscale exactly as the host build does. The expensive
...
/// Assemble the MomentumOp entirely on the device from a built host AmrCutCell + a device octree
/// view. The SDF samples / neighbour indices / fluid flags / advection CSR are staged to the device
/// (the SDF sampling stays host, like D2's openness); the stencil rebuild + operator merge run on
/// device. The advection is folded into the single CSR (op.hasAdv = false), matching host
/// assembleOperator + hostOp.
template <unsigned Bits>
MomentumOp assembleMomentum(const AmrCutCell<Bits>& ccop, const BlockOctreeView<3, Bits>& ov,
  ...
  View<double> AC(Kokkos::view_alloc("mom::AC", Kokkos::WithoutInitializing),
                  static_cast<std::size_t>(n));
  View<double> off(Kokkos::view_alloc("mom::off", Kokkos::WithoutInitializing),
                   static_cast<std::size_t>(n) * 6);
  View<char> cut(Kokkos::view_alloc("mom::cut", Kokkos::WithoutInitializing),
                 static_cast<std::size_t>(n));
  ...
  geom.hasOpen = false;
  for (int d = 0; d < 3; ++d)
    geom.fineExt[d] = ccop.lap().fineExt()[d];
  ...
  Kokkos::parallel_for("amr::mom_diag", n, KOKKOS_LAMBDA(const Index i) { diagV(i) = e.diag(i); });
  return op;
}
