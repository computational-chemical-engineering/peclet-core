...
// tables) is a per-cell list of faces with a neighbour index and a coefficient. Assembling such a
// CSR in parallel is the standard three-step pattern:
...
// `operator()(Index i, Sink& s)` that calls `s(neighbourLeaf, coef)` once per face in a fixed
// order. The same functor drives both the count pass (a counting sink) and the fill pass (a writing
// sink), so the emit order is guaranteed identical between the two — exactly as the host assembler
...
/// CSR row offsets (size n+1) from a per-cell count functor, via one exclusive prefix scan, with
/// the total written to `nTotal`. The standalone scan half of S1 — for assemblers whose per-face
/// payload is richer than (nbr, coef) (e.g. the face-geometry tables), which then run their own
/// own-slice fill over these offsets. `CountFn` is device-callable: KOKKOS Index operator()(Index
/// i) const.
...
/// An assembled face-CSR: row offsets (size n+1), neighbour index + coefficient per face (size
/// nFaces).
struct Csr {
...
/// and once in the fill — the same two traversals the host assembler does). Offsets are written by
/// the scan's `final` pass; the fill writes each cell's own slice with no atomics.
template <class Emit>
Csr buildFaceCsr(Index n, const Emit& emit) {
  ...
  Kokkos::parallel_scan(
      ...
      KOKKOS_LAMBDA(const Index i, Index& partial, const bool final_pass) {
        ...
        if (final_pass)
          start(i) = partial;  // exclusive prefix: offset before adding this cell
        partial += c;
      });
