...
// grid solver runs as ONE block: every ghost cell is a periodic self-copy onto the same block, so
// there are no remote neighbours. The only collectives ever EXECUTED at size 1 are the trivial ones
// -- rank=0, size=1, Allreduce/Bcast are local identities. The point-to-point and
// neighbourhood-collective calls exist only so the single code path compiles and links; they are
// never reached at size 1.
//
// This keeps "one code": the same GridHaloTopology / GridHalo / NbxEngine run, with MPI replaced by
// these stubs. Build WITH MPI (the default) and this header is never included -- see
// peclet/core/common/mpi.hpp.
#pragma once
...
inline int MPI_Initialized(int* f) {
  *f = 1;
  return MPI_SUCCESS;
}  // pretend initialised (no real MPI)
inline int MPI_Finalize() {
  return MPI_SUCCESS;
}
inline int MPI_Finalized(int* f) {
  *f = 0;
  return MPI_SUCCESS;
}
...
inline int MPI_Allreduce(const void* sbuf, void* rbuf, int count, MPI_Datatype dt, MPI_Op,
                         MPI_Comm) {
  if (sbuf != rbuf)
    std::memcpy(rbuf, sbuf, static_cast<size_t>(count) * static_cast<size_t>(dt));
  return MPI_SUCCESS;
}
...
// --- never reached at size 1 (no remote neighbours); present only to compile/link the one code
// path ---
...
inline int MPI_Irecv(void*, int, MPI_Datatype, int, int, MPI_Comm, MPI_Request*) {
  return MPI_SUCCESS;
}
...
inline int MPI_Ibarrier(MPI_Comm, MPI_Request*) {
  return MPI_SUCCESS;
}
...
inline int MPI_Neighbor_alltoallv(const void*, const int*, const int*, MPI_Datatype, void*,
