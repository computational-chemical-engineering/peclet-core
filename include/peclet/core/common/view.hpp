...
std::vector<std::remove_const_t<typename V::value_type>> toVector(const V& view) {
  ...
  if (view.size()) {
    ...
    // SoA has grown past numReal): a cross-space (device->host) deep_copy of a strided layout has
    // no valid copy mechanism and aborts. Repack into a CONTIGUOUS buffer in the SOURCE space first
    // (same-space deep_copy handles the strided->contiguous gather via a kernel), then hop to host.
    // A no-op memcpy when `view` is already contiguous. Only extent(0) is a runtime dimension for
    // the suite's particle getters (float* / float*[N]).
